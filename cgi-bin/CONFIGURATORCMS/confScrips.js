...
$(document).ready(function(){
	...
	sendTrigger("button", "http://localhost/configur-cms/controler", "cgi");	
...
app.controller('myCtrl', function($scope, $http){
	...
	$http({
        method : "GET",
        url : "http://localhost/configur-cms/controler.cgi?action=service&"
	...
	$http({
        	method : "GET",
	        url : "http://localhost/configur-cms/controler.cgi?action=categs&"
    	}).then(function mySuccess(response) {
	...
	$http({
        	method : "GET",
	        url : "http://localhost/configur-cms/controler.cgi?action=read_numb&"
    	}).then(function mySuccess(response) {
        	//$scope.selCategs = [];
		...
		$("#spec_price").text(" YOU CAN COMBINE " + conflimit[0] + " PRODUCTS FOR JUST "); 
	}, function myError(response) {
        	$scope.cont = response.statusText;
                console.log(JSON.stringify(response.statusText));
	});
	...
	$http({
        	method : "GET",
	        url : "http://localhost/configur-cms/controler.cgi?action=price&"
    	}).then(function mySuccess(response) {
        	//$scope.selCategs = [];
		var conflimit = response.data;
		...
		$("#spec_price").text( txt + " " + conflimit[0][0] + " EURO");
	}, function myError(response) {
        	$scope.cont = response.statusText;
                console.log(JSON.stringify(response.statusText));
	});
